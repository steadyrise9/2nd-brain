...
def test_edit_file_crud_and_read_modes():
    ...
    tool = EditFile()
    try:
        assert tool.run(_ctx(), operation="create", path=str(path), content="hello\nworld\n").success
        ...
        assert tool.run(_ctx(), operation="replace", path=str(path), old_text="world", new_text="brain").success
        assert tool.run(_ctx(), operation="append", path=str(path), content="!").success
        assert path.read_text(encoding="utf-8") == "hello\nbrain\n!"
        assert tool.run(_ctx(), operation="delete", path=str(path)).success
